    ...
    public class CustomerRepository : ICustomerRepository
    {
        ...
        public async Task<IEnumerable<CustomerInfo>> GetCustomer()
        {
            ...
            const string sql = @"
                SELECT 
                u.id,
                ui.last_name+' '+ui.first_name AS Name, 
                ui.phone_number AS PhoneNumber, u.user_name, u.email, r.role_name
                FROM users AS u
                LEFT JOIN customers AS ui on ui.user_id = u.id
                LEFT JOIN user_roles AS ur on u.id = ur.user_id
                LEFT JOIN roles AS r on r.id = u.id
				Group by u.id, ui.last_name+' '+ui.first_name ,ui.phone_number,u.user_name,u.email,r.role_name
            ";
